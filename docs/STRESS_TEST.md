...
Antes de optimizar, la prueba corta con 200 enemigos promedió 10,0 FPS y bajó a 8,2 FPS. El resultado final usa `LOD0` animado cerca del jugador, `LOD1` estático a distancia media, `LOD2` por `MultiMesh` separado por especie en distancia lejana, lógica distante menos frecuente y suspensión/liberación de modelos fuera de cámara. No se mantienen 200 esqueletos animados completos en el plano lejano. La cámara también suaviza por separado su punto de mira y evita atravesar edificios.

La aparición usa carga diferida: el proxy estático de especie se muestra inmediatamente y el FBX animado solo se prepara cuando el enemigo entra en el radio cercano. Los enemigos terrestres se desplazan entre edificios, atacan al alcanzar su perímetro y luego eligen otro objetivo; los voladores orbitan las construcciones.
